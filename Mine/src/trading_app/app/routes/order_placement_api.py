...
def exit_all():
    ...
    """
    from trading_app.app.order_placement.tg_call_engine import stop_all_calls
    ...
    try:
        ...
        # The orders are gone; the Telegram-call engine has to be told, or it
        # keeps managing legs that are no longer there and re-places a stop
...
# ── Telegram calls ───────────────────────────────────────────────────────
# The page's automatic trades. There is no arm route: the channel is the only
# way in (see app/order_placement/tg_calls_listener.py), and these routes are
# read-only — a call's legs are ordinary 'op' records, so the strip's ✕ and
# Exit all are the way out.
@order_placement_bp.route('/tg-calls', methods=['GET'])
@require_user_auth
def tg_calls_status():
    """The Telegram-call watcher, read-only: is the listener connected, is
    the engine up, which brokers a call would reach, and today's calls with
    the legs they placed. There is no arm route — the channel is the only way
    in — and no cancel route: a call's legs are ordinary 'op' records, so the
    strip's ✕ and Exit all are the way out."""
    from trading_app.app.order_placement import tg_call_engine as eng
    from trading_app.app.order_placement import tg_calls_listener as listener
    from trading_app.app.order_placement.tg_call_store import TgCallStore

    try:
        eng.remember_session(_user(), dict(session))
        calls = sorted(TgCallStore.get_today(),
                       key=lambda c: c.get('created_at', 0), reverse=True)
        for c in calls:
            c['legs'] = sorted(eng.call_records(c['id']),
                               key=lambda o: o.get('created_at', 0))
        return jsonify({'success': True,
                        'active': eng.is_active(_user()),
...
@require_user_auth
def tg_calls_history():
    """The auto-trader's P&L ledger: one row per closed call per broker,
    newest first, with totals. ``pnl_per_lot`` is the figure that compares
    across accounts of different size; ``pnl`` is that times the lots the
    account traded. ``?days=N`` windows it (default 30, 0 = everything)."""
    from trading_app.app.order_placement.tg_call_engine import history
    ...
    try:
        try:
            ...
            days = 30
        rows = history(days)
        booked = [r for r in rows if r.get('pnl') is not None]
        per_lot = [r['pnl_per_lot'] for r in booked if r.get('pnl_per_lot') is not None]
        totals = {
            'trades': len(rows),
            'booked': len(booked),
            'wins': sum(1 for r in booked if r['pnl'] > 0),
            'losses': sum(1 for r in booked if r['pnl'] < 0),
            'lots': sum(int(r.get('lots') or 0) for r in rows),
            'pnl': round(sum(r['pnl'] for r in booked), 2),
            'pnl_per_lot': round(sum(per_lot), 2),
            'avg_per_lot': round(sum(per_lot) / len(per_lot), 2) if per_lot else None,
            'incomplete': sum(1 for r in rows if not r.get('complete', True)),
        }
        by_day = {}
        for r in rows:
            d = by_day.setdefault(r.get('date'), {'date': r.get('date'), 'trades': 0,
                                                  'pnl': 0.0, 'pnl_per_lot': 0.0})
            d['trades'] += 1
            d['pnl'] = round(d['pnl'] + (r.get('pnl') or 0), 2)
            d['pnl_per_lot'] = round(d['pnl_per_lot'] + (r.get('pnl_per_lot') or 0), 2)
        return jsonify({'success': True, 'days': days, 'rows': rows, 'totals': totals,
                        'by_day': sorted(by_day.values(), key=lambda d: d['date'] or '',
                                         reverse=True)})
    except Exception as e:
        return _fail(e, 'tg_calls_history')
