...
class HighLowSignal:
    ...
    @staticmethod
    def get_sl_and_trailing_points(entry_price: float) -> Tuple[float, float]:
        """Calculate SL and trailing points based on entry price.
        ...
        Returns:
            Tuple of (initial_sl_points, trailing_sl_points)
        """
        if entry_price < 100:
            return 20.0, 10.0
        ...
        for i in range(1, min_len):
            ...
            if ce_open < pe_prev_high:
                ...
                if ce_open_candle <= pe_prev_high and ce_current > pe_prev_high and ce_low <= pe_prev_high and pe_low < ce_prev_low:
                    ...
                    sl_points, _ = self.get_sl_and_trailing_points(ce_current)
                    self.current_sl = ce_current - sl_points
                    self.in_trade = True
        ...
        for i in range(1, min_len):
            ...
            if pe_open < ce_prev_high:
                ...
                if pe_open_candle <= ce_prev_high and pe_current > ce_prev_high and pe_low <= ce_prev_high and ce_low < pe_prev_low:
                    ...
                    sl_points, _ = self.get_sl_and_trailing_points(pe_current)
                    self.current_sl = pe_current - sl_points
                    self.in_trade = True
    ...
    def execute_trade(self, entry_signal: Dict[str, Any], option_data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        # Get trailing SL points based on entry price
        _, trailing_sl_points = self.get_sl_and_trailing_points(entry_price)
        ...
        for i in range(entry_idx + 1, len(option_data)):
            ...
            # Trailing SL: Every trailing_sl_points of profit, trail SL by trailing_sl_points
            # Calculate profit based on current close
            profit = candle_close - entry_price
            if profit >= trailing_sl_points:
                # Number of completed trailing_sl_points-point increments
                increments = int(profit / trailing_sl_points)
                # New SL should be at: entry_price + (increments - 1) * trailing_sl_points
                # E.g., at trailing_sl_points profit: SL moves to entry (increments=1, so entry + 0*trailing_sl_points)
                # E.g., at 2*trailing_sl_points profit: SL moves to entry + trailing_sl_points (increments=2)
                new_sl = entry_price + (increments - 1) * trailing_sl_points
