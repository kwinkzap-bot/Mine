...
function _algoOptTiles(trade, live) {
    ...
    return [
        ...
        { label: 'Opt P&L (₹)', value: optPnl   != null ? _algoFmtInr(optPnl)   : '…',
          cls: _algoPnlCls(optPnl) },
    ];
}

// Which pair of price fields a trade books its ₹ P&L on: every option tab
// (2nd Candle) trades a bought option.
...
function _activeRenderPnl(rows, todayTrades) {
    ...
    const tiles = [
        { label: 'Total P&L (₹)',      value: inrFmt(total),      cls: _algoPnlCls(total) },
        { label: 'Realised (₹)',       value: inrFmt(realised),   cls: _algoPnlCls(realised) },
        { label: 'Unrealised (₹)',     value: openWithLive ? inrFmt(unrealised) : '—', cls: openWithLive ? _algoPnlCls(unrealised) : '' },
...
// Every option tab renders the same table; they differ in exactly one thing,
// so that is the only parameter: which delete handler the row button calls.
...
// ── Shared dashboard helpers (Active tab + per-algo performance cards) ────────

// Plugin: draw +/- ₹ labels above/below each breakdown bar.
const _algoBarLabelPlugin = {
    id: 'algoBarLabels',
...
function _algoPeriodKey(d, period) {
...
function _algoFmtInr(inr) {
    if (inr == null) return '…';
    return (inr >= 0 ? '+₹' : '-₹') + Math.abs(inr).toFixed(0);
}

function _algoPnlCls(val) {
    if (val == null) return '';
...
function _scRenderDashboard(trades) {
    ...
    const tiles = [
        ...
        { label: 'Net P&L (₹)',   value: inrFmt(netInr), cls: _algoPnlCls(netInr) },
        { label: 'Net Opt Pts',   value: ptsFmt(netPts), cls: _algoPnlCls(netPts) },
...
function _scRenderBreakdown(trades, period) {
    ...
    const groups = {};
    (trades || []).forEach(t => {
        const key = _algoPeriodKey(new Date(t.entry_time), period);
